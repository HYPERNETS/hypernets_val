    ...
    if contain_flag:  # if contain_flag==1
        ...
        if r >= 0 and r + 1 < lat.shape[0] and c >= 0 and c + 1 < lat.shape[1]:
            ...
            if processor.lower() in ['acolite', 'ac']:
                ...
                if "Rrs_443" in nc_sat.variables.keys():
                    rhow_443 = nc_sat.variables['Rrs_443'][:]*np.pi
                else:
                    rhow_443 = nc_sat.variables['Rrs_442'][:]*np.pi
                rhow_492 = nc_sat.variables['Rrs_492'][:]*np.pi
                if "Rrs_560" in nc_sat.variables.keys():
                    rhow_560 = nc_sat.variables['Rrs_560'][:]*np.pi
                else:
                    rhow_560 = nc_sat.variables['Rrs_559'][:]*np.pi
                rhow_665 = nc_sat.variables['Rrs_665'][:]*np.pi
                rhow_704 = nc_sat.variables['Rrs_704'][:]*np.pi
                if "Rrs_740" in nc_sat.variables.keys():
                    rhow_740 = nc_sat.variables['Rrs_740'][:]*np.pi
                else:
                    rhow_740 = nc_sat.variables['Rrs_739'][:]*np.pi
                if "Rrs_783" in nc_sat.variables.keys():
                    rhow_783 = nc_sat.variables['Rrs_783'][:]*np.pi
                else:
                    rhow_783 = nc_sat.variables['Rrs_780'][:]*np.pi
                rhow_833 = nc_sat.variables['Rrs_833'][:]*np.pi
                if "Rrs_865" in nc_sat.variables.keys():
                    rhow_865 = nc_sat.variables['Rrs_865'][:]*np.pi
                else:
                    rhow_865 = nc_sat.variables['Rrs_864'][:]*np.pi
                if "Rrs_1614" in nc_sat.variables.keys():
                    rhow_1614 = nc_sat.variables['Rrs_1614'][:]*np.pi
                else:
                    rhow_1614 = nc_sat.variables['Rrs_1610'][:]*np.pi
                if "Rrs_2202" in nc_sat.variables.keys():
                    rhow_2202 = nc_sat.variables['Rrs_2202'][:]*np.pi
                else:
                    rhow_2202 = nc_sat.variables['Rrs_2186'][:]*np.pi
                nbands = 11

            #             AOT_0865p50 = nc_sat.variables['T865'][:]
            if processor.lower() in ['c2rcc']:
                try:
                    rhow_443 = nc_sat.variables['rrs_B1'][:, :]*np.pi
                    rhow_492 = nc_sat.variables['rrs_B2'][:, :]*np.pi
                    rhow_560 = nc_sat.variables['rrs_B3'][:, :]*np.pi
                    rhow_665 = nc_sat.variables['rrs_B4'][:, :]*np.pi
                    rhow_704 = nc_sat.variables['rrs_B5'][:, :]*np.pi
                    rhow_740 = nc_sat.variables['rrs_B6'][:, :]*np.pi
                    rhow_783 = nc_sat.variables['rrs_B7'][:, :]*np.pi
                    # rhow_833 = np.zeros_like(Rrs443_c) # C2RCC does not provide this band
                    rhow_865 = nc_sat.variables['rrs_B8A'][:, :]*np.pi
                    nbands = 8
                except KeyError:
                    return
            ...
            ofname = "{}/{}".format(path_output, file).split('__ACI.nc')[
                           0] + '_extract_' + station_name + '_' + processor + '.nc'
...
#            satellite_PDU.long_name = "S2A source PDU name"
            ...
            satellite_Rrs = new_EXTRACT.createVariable('satellite_Rrs', 'f4',
                                                       ...
                                                       fill_value=-999, zlib=True, complevel=6)
            if nbands == 8:
                satellite_Rrs[0, 0, :, :] = [ma.array(rhow_443[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                satellite_Rrs[0, 1, :, :] = [ma.array(rhow_492[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                satellite_Rrs[0, 2, :, :] = [ma.array(rhow_560[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                satellite_Rrs[0, 3, :, :] = [ma.array(rhow_665[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                satellite_Rrs[0, 4, :, :] = [ma.array(rhow_704[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                satellite_Rrs[0, 5, :, :] = [ma.array(rhow_740[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                satellite_Rrs[0, 6, :, :] = [ma.array(rhow_783[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                satellite_Rrs[0, 7, :, :] = [ma.array(rhow_865[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
            else:
                if nbands == 11:
                    satellite_Rrs[0, 0, :, :] = [
                        ma.array(rhow_443[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                    satellite_Rrs[0, 1, :, :] = [
                        ma.array(rhow_492[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                    satellite_Rrs[0, 2, :, :] = [
                        ma.array(rhow_560[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                    satellite_Rrs[0, 3, :, :] = [
                        ma.array(rhow_665[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                    satellite_Rrs[0, 4, :, :] = [
                        ma.array(rhow_704[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                    satellite_Rrs[0, 5, :, :] = [
                        ma.array(rhow_740[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                    satellite_Rrs[0, 6, :, :] = [
                        ma.array(rhow_783[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                    satellite_Rrs[0, 7, :, :] = [
                        ma.array(rhow_833[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                    satellite_Rrs[0, 8, :, :] = [
                        ma.array(rhow_865[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                    satellite_Rrs[0, 9, :, :] = [
                        ma.array(rhow_1614[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]
                    satellite_Rrs[0, 10, :, :] = [
                        ma.array(rhow_2202[start_idx_y:stop_idx_y, start_idx_x:stop_idx_x]) / np.pi]

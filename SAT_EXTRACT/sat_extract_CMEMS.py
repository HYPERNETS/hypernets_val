import argparse,configparser,os, sys, pytz, subprocess, __init__
...
class SatExtractCMEMS:
    ...
    def get_files_day(self,datehere, input_path_info, sat_extract_options):
        ...
        name = dataset_name_file.replace('$DATE$', strdate)
    ...
    def run_extract_day(self,extract_options, extract_info, lat_array, lon_array, output_path, overwrite):
        ...
        dataset_name_format_date = extract_options['dataset_name_format_date']
        strdate  = extract_info['satellite_time'][0].strftime(dataset_name_format_date)

        if extract_options['is_utm']:
            dataset_name_file = extract_options['dataset_name_file']
            dataset_name_file = dataset_name_file.replace('$DATE$', strdate)
        ...
        for idx in range(len(sat_file_indices_used)):
            ...
            for itime in range(ntimes_here):
                ...
                if extract_options['otherfiles_var_list'] is not None:
                    ...
                    for name_of in ofvl:
                        name_here = name_of.replace('$DATE$',strdate)
                        if extract_options['is_utm']:
    ...
    def create_rrs_variables(self,newExtract,extract_info,options,cmems_source):
        ...
        array_rrs = cmems_source.get_spectral_array(options['rrs_var_list'],options['rrs_format'],extract_info['n_bands'],extract_info['limits'])
        ...
        if options['rrs_unc_var_list'] is None:
            return True

        array_rrs_unc = cmems_source.get_spectral_array(options['rrs_unc_var_list'], options['rrs_unc_format'],extract_info['n_bands'],extract_info['limits'])
        if array_rrs_unc is None:
            return False
        else:
...
class SatSourceCMEMS:
    ...
    def __init__(self, path_source,extract_options):
        if os.path.isfile(path_source):
            ...
            print(f'[ERROR] CMEMS source could not be  started with path: {self.path_source}')
    ...
    def check_dataset(self,extract_options):
        ...
        if extract_options is not None:
            try:
                dim_lat = extract_options['dim_lat']
                dim_lon = extract_options['dim_lon']
                dim_time = extract_options['dim_time']
            except:
                pass
        try:
        ...
        if not dim_lat in dataset.dimensions:
            print(f'[ERROR] Latitude dimension {dim_lat} in not available in the file {self.path_source}')
            dv_avaialable = False
        if not dim_lat in dataset.variables:
            print(f'[ERROR] Latitude variable {dim_lat} in not available in the file {self.path_source}')
            dv_avaialable = False
        if not dim_lon in dataset.dimensions:
            print(f'[ERROR] Longitude dimension {dim_lon} in not available in the file {self.path_source}')
            dv_avaialable = False
        if not dim_lon in dataset.variables:
            print(f'[ERROR] Longitude variable {dim_lon} in not available in the file {self.path_source}')
            dv_avaialable = False
        if not dim_time in dataset.dimensions:
            print(f'[ERROR] Time dimension {dim_time} in not available in the file {self.path_source}')
            dv_avaialable = False
        if not dim_time in dataset.variables:
            print(f'[ERROR] Time variable {dim_time} in not available in the file {self.path_source}')
            dv_avaialable = False
        dataset.close()
        if dv_avaialable:
    ...
    def get_spectral_array(self,var_list,name_format,nbands,window):
        if not self.valid:
            return None

        ##checking number of bands
        if 0 < nbands != len(var_list):
            print(f'[ERROR] Inconsistency between the number of bands {nbands} andt number of variables ({len(nbands)}) defined in the configuration file')
            return None
    ...
    def get_nonspectral_array(self,name_var,window):
        ...
        if name_var not in nc_sat.variables:
            ...
            attrs = {}
            for at in nc_sat.variables[name_var].ncattrs():
                if at=='_FillValue' or at == 'scale_factor' or at == 'add_offset':
                    continue
                attrs[at] = nc_sat.variables[name_var].getncattr(at)
                if (at == 'valid_min' or at == 'valid_max') and 'scale_factor' in nc_sat.variables[name_var].ncattrs() and 'add_offset' in nc_sat.variables[name_var].ncattrs():
                    attrs[at] = (attrs[at] * nc_sat.variables[name_var].scale_factor) + nc_sat.variables[name_var].add_offset
...
def download_doors_sources(options):
    ...
    if not options.has_section(section):
        ...
        return
    compulsory_options = ['sat_source_dir', 'dataset_name_file', 'dataset_name_format_date']
    options_dict = {}
    ...
    if 'path_csv' not in options_dict.keys() and 'start_date' in options_dict.keys() and 'end_date' in options_dict.keys():
        ##download using a date range instead of a date list
        try:
            start_date = dt.strptime(options_dict['start_date'], '%Y-%m-%d')
        except:
            print(f'[ERROR] start_date {options_dict["start_date"]} is not in the correct format yyyy-mm-ddd')
            return
        try:
            end_date = dt.strptime(options_dict['end_date'], '%Y-%m-%d')
        except:
            print(f'[ERROR] end_date {options_dict["end_date"]} is not in the correct format yyyy-mm-ddd')
            return
        ...
        while work_date<=end_date:
            ...
            if sfile is not None:
                ...
                print(f'[WARNING] DOORS file could not be found for date: {work_date.strftime("%Y-%m-%d")}')
            work_date = work_date + timedelta(hours=24)

        return

    if not 'path_csv' in options_dict.keys():
        print(f'[ERROR] Date list to download should be incluided in the configuration file, section doors_download, as:')
        print(f'[ERROR]  using path_csv to define a file with a date list in format yyyy-mm-dd in column Date   or')
        print(f'[ERROR]  or using start_date and end_date (both in format yyyy-mm-dd)')
        return
        ...
        for name in os.listdir(path_csv):
            ...
            for date_str in only_date_array_unique:
                ...
                if extract_options['use_single_file']:
                    fproduct = get_cmems_product_day_strict(path_source, org, dt.strptime(date_str, '%Y-%m-%d'),
                ...
                for idx, row in df.iterrows():
                    ...
                    if extract_options['use_single_file']:
                        ...
                        if extract_options['dataset_var_list'] is not None:
                            newExtract = add_variable_single(newExtract, extract_info,
                                                             extract_options['dataset_var_list'],

...
def plot_spectra(index_mu,wavelenghts,ydata,platform,sat_time,insitu_time):

    legend = [f"S3{platform}-{sat_time.strftime('%d-%m-%Y %H:%M')}",f"Hypstar-{insitu_time.strftime('%d-%m-%Y %H:%M')}"]
    row_names = []
...
def config_reader(FILEconfig):
    ...
    if int(options['satellite_options']['window_size']) > int(options['satellite_options']['miniprods_size']) or int(
            options['satellite_options']['window_size']) % 2 == 0:
        print(
            'windows_size must be an odd number between 1 and %d' % int(options['satellite_options']['miniprods_size']))
        sys.exit()
    return options
...
class PANTHYR_class(object):
    def __init__(self, options):
        ...
        self.satellite_Nbands = 16  # config_MDB.n_sensor_bands #No. OLCI bands
        self.insitu_Nbands = 1605  # config_MDB.Rrs_Nbands_PANTHYR
    ...
    def MDB_PANTHYR_reader(self, options):
        ...
        columns = ['datetime', 'PDU', 'insitu_filename', 'OZA', 'SZA', 'LWN_ins_400.00', 'LWN_ins_412.50',
                   'LWN_ins_442.50', \
                   'LWN_ins_490.00', 'LWN_ins_510.00', 'LWN_ins_560.00', 'LWN_ins_620.00', 'LWN_ins_665.00',
                   'LWN_ins_673.75', \
                   'LWN_ins_681.25', 'LWN_ins_708.75', 'LWN_ins_753.75', 'LWN_ins_778.75', 'LWN_ins_865.00',
                   'LWN_ins_885.00', \
                   'LWN_ins_1020.50', 'LWN_sat_400.00', 'LWN_sat_412.50', 'LWN_sat_442.50', 'LWN_sat_490.00',
                   'LWN_sat_510.00', \
                   'LWN_sat_560.00', 'LWN_sat_620.00', 'LWN_sat_665.00', 'LWN_sat_673.75', 'LWN_sat_681.25',
                   'LWN_sat_708.75', \
                   'LWN_sat_753.75', 'LWN_sat_778.75', 'LWN_sat_865.00', 'LWN_sat_885.00', 'LWN_sat_1020.50',
                   'Rrs_ins_400.00', \
                   'Rrs_ins_412.50', 'Rrs_ins_442.50', 'Rrs_ins_490.00', 'Rrs_ins_510.00', 'Rrs_ins_560.00',
                   'Rrs_ins_620.00', \
                   'Rrs_ins_665.00', 'Rrs_ins_673.75', 'Rrs_ins_681.25', 'Rrs_ins_708.75', 'Rrs_ins_753.75',
                   'Rrs_ins_778.75', \
                   'Rrs_ins_865.00', 'Rrs_ins_885.00', 'Rrs_ins_1020.50', 'Rrs_sat_400.00', 'Rrs_sat_412.50',
                   'Rrs_sat_442.50', \
                   'Rrs_sat_490.00', 'Rrs_sat_510.00', 'Rrs_sat_560.00', 'Rrs_sat_620.00', 'Rrs_sat_665.00',
                   'Rrs_sat_673.75', \
                   'Rrs_sat_681.25', 'Rrs_sat_708.75', 'Rrs_sat_753.75', 'Rrs_sat_778.75', 'Rrs_sat_865.00',
                   'Rrs_sat_885.00', \
        ...
        for MDBpath in input_list:
            ...
            if nc.satellite + nc.platform == str(options['satellite_options']['satellite']).replace(" ",
                                                                                                    "").upper() + str(
                options['satellite_options']['platform']).replace(" ", "") \
                ...
                if time_difference[ins_time_index] < delta_t * 60 * 60 \
                    ...
                    for sat_band_index in range(0, satellite_rhow.shape[0]):
                        ...
                        if float(options['Filtering_options']['valid_min_pixel']) == 1:  # 100% like Zibordi
                            ...
                            cond_min_valid_pxs = numberValid > (
                                    float(options['Filtering_options']['valid_min_pixel']) * NTP + 1)
                        ...
                        if cond_min_valid_pxs:
                            ...
                            if debug:
                                print(
                                    f'in situ: {insitu_rrs[ins_band_index, ins_time_index]}; sat: {curr_sat_box_mean}')
            ...
            if check_cv560 and curr_sat_box_cv_560 <= float(options['Filtering_options']['cv_max']):
                ...
                for sat_band_index in range(len(curr_bands) - 1):
                    if curr_bands[sat_band_index] in satellite_BRDF_bands_list and options['satellite_options'][
                        'BRDF'] == 'T':
                        mfc = 'Gray'
                        lw = 1.5
                    else:
            ...
            # [LUIS] CODE
            insitu_bands = nc.variables['insitu_original_bands'][:]  # insitu_bands(insitu_bands)
            satellite_bands = nc.variables['satellite_bands'][:]
            self.insitu_bands = insitu_bands
            self.satellite_bands = satellite_bands

            ndays = len(nc.dimensions['satellite_id'])
            sat_times = nc.variables['satellite_time']
            sat_proc_version_str = nc.satellite_proc_version
            # sat_PDU = nc.varibles['satellite_PDU']

            ##loop days (MU)
            for iday in range(ndays):
                check_insitu = True
                time_difference = nc.variables['time_difference'][iday]
                ins_time_index = np.argmin(np.abs(time_difference))


                # flags mask
                flagging = flag.Class_Flags_OLCI(nc.variables['satellite_WQSF'].flag_masks,
                                                 nc.variables['satellite_WQSF'].flag_meanings)
                satellite_WQSF = nc.variables['satellite_WQSF'][iday, r_s:r_e, c_s:c_e]
                if (str(flag_list[0])) != 'None':
                    mask = flagging.Mask(satellite_WQSF, flag_list)
                    mask[np.where(mask != 0)] = 1
                else:
                ...
                # if time_difference[ins_time_index] < delta_t * 60 * 60 and cond_min_valid_pxs:

                if np.abs(time_difference[ins_time_index]) < delta_t and cond_min_valid_pxs:

                    sat_time = datetime.datetime(1970, 1, 1) + datetime.timedelta(seconds=int(sat_times[iday]))
                    date_list.append(sat_time.strftime('%Y%m%d'))  # to avoid multiple MUs per day

                    satellite_rhow = nc.variables['satellite_Rrs'][iday]
                    # satellite_BRDF_rhow = nc.variables['satellite_BRDF_rhow'][:]
                    insitu_rrs = nc.variables['insitu_rhow'][iday]
                    insitu_rrs = insitu_rrs / np.pi  # transform from rhow to Rrs
                    insitu_finame_list = nc.variables['insitu_filename'][iday]
                    insitu_times_list = nc.variables['insitu_time'][iday]
                    ...
                    for sat_band_index in range(0, satellite_rhow.shape[0]):
                        ...
                        if cond_min_valid_pxs:
                            ...
                            # in situ. curr_ins_rrs is a list with N=number of bands elements
                            ins_value = insitu_rrs[ins_band_index, ins_time_index]
                            if isinstance(ins_value, np.ma.core.MaskedConstant):
                                check_insitu = False

                            insitu_filename = insitu_finame_list[ins_time_index]
                            insitu_time = insitu_times_list[ins_time_index]
                            insitu_time = datetime.datetime(1970, 1, 1) + datetime.timedelta(seconds=int(insitu_time))
                            curr_ins_rrs.append(ins_value)
                            # curr_sat_rrs_mean.append(curr_sat_box_mean / np.pi)  # transform rhow to Rrs
                            curr_sat_rrs_mean.append(curr_sat_box_mean)  # it is already
                            curr_bands.append(wl)
                            if debug:
                                print(
                                    f'in situ: {insitu_rrs[ins_band_index, ins_time_index]}; sat: {curr_sat_box_mean}')
                ...
                if check_insitu and check_cv560 and curr_sat_box_cv_560 <= float(options['Filtering_options']['cv_max']):
                    ...
                    df_matchups = df_matchups.append( \
                        {'datetime': sat_time.strftime('%Y%m%dT%H%M'), \
            ...
            print(['LUIS MU', N_MUs, matchups_ins_array.shape, matchups_sat_array.shape])

            # scatter plots
            # plotting all bands
            for sat_band_index in range(n_bands):
                # if curr_bands[sat_band_index] in satellite_BRDF_bands_list and options['satellite_options']['BRDF'] == 'T':
                #     mfc = 'Gray'
                #     lw = 1.5
                # else:
                ...
                ns_band_index = np.argmin(np.abs(curr_bands[sat_band_index] - insitu_bands))
                ins_band = insitu_bands[ns_band_index]
                sat_band = curr_bands[sat_band_index]

                # print(['LUIS PLOT ', sat_band_index, sat_band, ins_band])
                # print(options['plot_options']['to_plot'])

                curr_ins_rrs = matchups_ins_array[:, sat_band_index]
                curr_sat_rrs_mean = matchups_sat_array[:, sat_band_index]
                # print(curr_ins_rrs.shape)
                # print(curr_sat_rrs_mean.shape)
                plt.scatter(curr_ins_rrs, curr_sat_rrs_mean, c=color_dict[f'{curr_bands[sat_band_index]:.2f}'],
                            edgecolors='gray', linewidths=1.5)

                # if options['plot_options']['to_plot'] == 'rhow':
                #     plt.scatter(curr_ins_rrs[sat_band_index] * np.pi,
                #         curr_sat_rrs_mean[sat_band_index] * np.pi, \
                #         c=color_dict[f'{curr_bands[sat_band_index]:.2f}'], edgecolors=mfc,linewidths=lw)
                # elif options['plot_options']['to_plot'] == 'Rrs':
                #     plt.scatter(curr_ins_rrs[sat_band_index], curr_sat_rrs_mean[sat_band_index], \
                #             c=color_dict[f'{curr_bands[sat_band_index]:.2f}'], edgecolors=mfc,linewidths=lw)
        ...
        for sat_band_index in range(len(curr_bands)):
            ...
            ins_band = insitu_bands[ins_band_index]
            sat_band = curr_bands[sat_band_index]
            df = plot_scatter(x, y, sat_band, ins_band, output_directory, 'ba', 'OLCI', 'Venise_PAN',
                              sat_proc_version_str, \

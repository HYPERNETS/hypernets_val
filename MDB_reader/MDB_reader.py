...
def make_validation_single_MDB():
    ...
    # wllist = reader.mfile.wlref
    # reader.mfile.qc_insitu.set_wllist_using_wlref(wllist)
...
def make_togheter_ab(ac):
    ...
    for idx in range(len(params)):
        ...
        # pl.start_plot()
        # pl.xdata = wl
        # pl.plot_single_line(param_values_a, 'blue', '-', 2, 'o')
        # pl.plot_single_line(param_values_b, 'red', '-', 2, 'o')
        # pl.set_xaxis_title('Wavelength (nm)')
        # pl.set_yaxis_title(params_file[idx])
        # str_legend = [f'S3A', f'S3B']
        # pl.set_legend(str_legend)
        # pl.set_grid()
        # pl.set_tigth_layout()
        # pl.save_plot(os.path.join(path_out, params_file[idx] + '.jpg'))
        # pl.close_plot()
...
def compute_sam_impl(dfvalid, dfsam, wl_potentiallist):
    ...
    for index, row in dfvalid.iterrows():
        ...
        if index_mu_here != index_mu_ref:
            if index_mu_ref != -1:
                Topline = np.dot(np.array(insitu_here), np.array(sat_here))#TOTAL(double(class_rrs) * double(input_rrs))

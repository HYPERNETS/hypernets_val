...
class MDBFile:
    ...
    def __init__(self, file_path):
        ...
        print(f'[INFO] Starting MDBFile: {os.path.basename(file_path)}')
        try:
        ...
        if self.VALID:
            self.n_mu_total = len(self.nc.dimensions['satellite_id'])
            self.sat_times = [dt.fromtimestamp(float(st)).astimezone(timezone.utc) for st in self.nc.variables['satellite_time'][:]]



        # if not self.VALID:
        #     print(
        #         f'[WARNING] MDB File: {file_path} is not a valid MDB file with the correct structure for spectral match-ups')
        #     return
        #
        # if self.VALID:
        ...
        #     self.n_instrument = -1
        #     if 'insitu_instrument_id' in self.variables:
        #
        #         # if isinstance(self.variables['insitu_instrument_id'].flag_values,np.ndarray):
        #         if not np.isscalar(self.variables['insitu_instrument_id'].flag_values):
        #             self.n_instrument = len(self.variables['insitu_instrument_id'].flag_values)
        #         else:
        #             self.n_instrument = 1
    ...
    def check_structure(self):
        ...
        for var in VAR_NAMES:
            if var not in self.nc.variables:
                print(f'[WARNING] Variable: {var} is not available in MDB file')
                return False

        for dim in DIMENSION_NAMES:
            if dim not in self.nc.dimensions:
                if dim == 'insitu_original_bands' and 'insitu_bands' in self.nc.dimensions:
                    pass
                else:
                    print(f'[WARNING] Dimension: {dim} is not available in MDB file')
                    return False
    ...
    #     print(f'[INFO] Completed')
    #
    # def get_name_mu_single_variable(self, new_MDB, variable, overwrite, add_new, suffix_new):
    #     variable_mu = f'mu_{variable}'
    #     if suffix_new is not None:
    #         variable_mu = f'{variable_mu}_{suffix_new}'
    #     create_new = True
    #     skip = False
    #     if variable_mu in new_MDB.variables:
    #         if not overwrite and not add_new:
    #             print(f'[WARNING] Variable {variable_mu} already exists. Skipping')
    #             skip = True
    #             create_new = False
    #         elif overwrite and not add_new:
    #             print(f'[WARNING] Variable {variable_mu} already existed. It will be overwritten')
    #             skip = False
    #             create_new = False
    #         elif add_new:
    #             if suffix_new is None:
    #                 index = 1
    #                 variable_mu_n = f'{variable_mu}_{index}'
    #                 while variable_mu_n not in new_MDB.variables:
    #                     index = index + 1
    #                     variable_mu_n = f'{variable_mu}_{index}'
    #                 variable_mu = variable_mu_n
    #             else:
    #                 variable_mu = f'{variable_mu}_{suffix_new}'
    #                 if variable_mu in new_MDB.variables:
    #                     if overwrite:
    ...
    #                     else:
    #                         print(
    #                             f'[ERROR] Variable: {variable_mu} already exists. Please use a different suffix or the overwrite option')
    #                         variable_mu = None
    #     return variable_mu, create_new, skip
    #
    # def create_mu_single_variables(self, new_MDB, overwrite, add_new, suffix_new):
    #     if self.qc_single is None:
    #         print(f'[ERROR] QC Single should be defined using a configuration file with section QC_SINGLE')
    #         return False
    #     # VARIABLE SAT
    #     variable_sat = self.qc_single.satellite_variable
    ...
    #
    #     # VARIABLE INS
    #     variable_ins = self.qc_single.insitu_variable
    ...
    #     if variable_ins_mu is None: return False
    #     if variable_ins_mu_id is None: return False
    #
    #     fill_value = -999.0
    #
    #     if create_sat:
    #         print(f'[INFO] Creating satellite variable: {variable_sat_mu}')
    #         new_var_sat = new_MDB.createVariable(variable_sat_mu, 'f4', ('mu_id',), zlib=True, complevel=6,
    #                                              fill_value=fill_value)
    #     else:
    #         print(f'[INFO] Retrieving in situ variable: {variable_ins_mu}')
    #         new_var_sat = new_MDB.variables[variable_sat_mu]
    #
    #     if create_ins:
    #         print(f'[INFO] Creating insitu variable: {variable_ins_mu}')
    #         new_var_ins = new_MDB.createVariable(variable_ins_mu, 'f4', ('mu_id',), zlib=True, complevel=6,
    #                                              fill_value=fill_value)
    #     else:
    #         print(f'[INFO] Retrieving insitu variable: {variable_ins_mu}')
    #         new_var_ins = new_MDB.variables[variable_ins_mu]
    #
    #     if create_ins_id:
    ...
    #         return False
    #     if nmu != self.variables[variable_ins].shape[0]:
    #         return False
    #
    #     for index_mu in range(nmu):
    ...
    #
    #     ##CHECK IF EXIST insitu_instrument_id
    #     if 'insitu_instrument_id' in self.variables:
    #         self.qc_insitu.instrument_id_array = self.variables['insitu_instrument_id'][index_mu]
    #     else:
    #         self.qc_insitu.instrument_id_array = None
    #
    #
    ...
    #
    # def prepare_df_validation_new(self):
    ...
    #     if 'insitu_instrument_id' in self.variables:
    #         self.qc_insitu.instrument_id_array = self.variables['insitu_instrument_id'][:]
    #     # index_tot = 0
    #     # nmu_valid = 0
    ...
    #
    #
    #     print(f'[INFO] Checking time differences....')
    ...
    #     satellite_time_r = np.repeat(satellite_time,self.n_insitu_day).reshape(insitu_time.shape)
    #     time_diff_computed = np.abs(satellite_time_r-insitu_time)
    #     check_time_diff = (time_diff_computed==time_diff_var).all()
    #     if not check_time_diff:
    #         print(f'[WARNING] Unexpected inconsistencies between the computed time difference and the time difference variable. Using the computed time difference')
    ...
    #
    #     print('[INFO] Getting match-ups combining satellite quality control + in insitu quality control + time difference')
    ...
    #     insitu_rrs_nearest = self.qc_insitu.get_spectra_rrs(min_insitu_id)
    #     insitu_rrs_valid = self.qc_insitu.get_spectra_rrs(min_insitu_id_valid,valid_sat_id=valid_mu_final)
    #     if 'insitu_Rrs_unc' in self.variables.keys():
    #         insitu_rrs_unc_nearest = self.qc_insitu.get_spectra_unc(min_insitu_id)
    #         #insitu_rrs_unc_valid = self.qc_insitu.get_spectra_unc(min_insitu_id_valid, valid_sat_id=valid_mu_final)
    #     else:
    #         insitu_rrs_unc_nearest = np.ma.masked_all((self.n_mu_total,nbands))
    #     print(f'[INFO] Retrieved {insitu_rrs_nearest.shape[0]}  in situ spectra with {insitu_rrs_nearest.shape[1]} bands')
    #     print(f'[INFO] Retrieved {insitu_rrs_valid.shape[0]} valid in situ spectra with {insitu_rrs_valid.shape[1]} bands')
    #
    #     print(f'[INFO] Started validation data frame creation...')
    #     sat_rrs_all = self.qc_sat.qc_sat_results['reported_rrs']
    #     sat_rrs_all_unc = self.qc_sat.qc_sat_results['reported_rrs_unc']
    #
    #     satellite_time_str = [dt.fromtimestamp(x).astimezone(timezone.utc).strftime('%Y-%m-%d %H:%M') for x in satellite_time]
    #
    #     insitu_time_nearest = insitu_time[np.arange(self.n_mu_total).astype(np.int32),min_insitu_id]
    #     insitu_time_nearest_str = [dt.fromtimestamp(x).astimezone(timezone.utc).strftime('%Y-%m-%d %H:%M') for x in insitu_time_nearest]
    #
    #     time_diff_computed_nearest = time_diff_computed[np.arange(self.n_mu_total).astype(np.int32),min_insitu_id]/3600
    #
    #     valid_str = np.array([False]*self.n_mu_total)
    #     valid_str[valid_mu_final==1] = True
    #
    #     n_valid_bands = np.ma.count(insitu_rrs_nearest, axis=1)
    #     spectrum_complete = n_valid_bands == nbands
    #     nmu_valid = np.sum(valid_str)
    #     spectrum_complete_valid = spectrum_complete * valid_str
    #     nmu_valid_complete = np.sum(spectrum_complete_valid)
    #
    #     status = np.ones(self.n_mu_total).astype(np.int32)
    #     status[time_diff_valid_mu==0]=-3
    #     status[valid_mu_ins==0]=-4
    #     status[valid_mu_sat==0]=-6
    #     status[spectrum_complete==False] = -5
    #
    #     data_df = {
    #         'Index': np.arange(ntot).astype(np.int32),
    #         'Index_MU': np.repeat(np.arange(self.n_mu_total).astype(np.int32),nbands),
    #         'Index_Band': np.tile(np.arange(nbands).astype(np.int32),self.n_mu_total),
    #         'Sat_Time': np.repeat(satellite_time_str, nbands),
    #         'Ins_Time': np.repeat(insitu_time_nearest_str,nbands),
    #         'Time_Diff': np.repeat(time_diff_computed_nearest,nbands),
    #         'Wavelenght': np.tile(self.wlref,self.n_mu_total),
    #         'Ins_Rrs': np.reshape(insitu_rrs_nearest,(ntot,)).filled(fill_value=-999),
    #         'Sat_Rrs': np.reshape(sat_rrs_all,(ntot,)).filled(fill_value=-999),
    #         'Ins_Rrs_unc': np.reshape(insitu_rrs_unc_nearest,(ntot,)).filled(fill_value=-999),
    #         'Sat_Rrs_unc': np.reshape(sat_rrs_all_unc,(ntot,)).filled(fill_value=-999),
    #         'Valid': np.repeat(valid_str,nbands),
    #         'status': np.repeat(status,nbands)
    #     }
    #
    #
    #
    #     self.df_validation = pd.DataFrame(data=data_df)
    #
    #
    #     self.df_validation_valid = self.df_validation[self.df_validation['Valid']][:]
    #     print(f'[INFO] Validation data frame creation completed.')
    #
    #     print(f'[INFO] Started summary data frames creation...')
    #     key_site = 'site'
    #     if key_site not in self.info.keys() and 'insitu_site_name' in self.info.keys():
    #         key_site = 'insitu_site_name'
    #     data_mu_df = {
    #         'Index_MU': np.arange(self.n_mu_total).astype(np.int32),
    #         'Sat_Time': satellite_time_str,
    #         'Ins_Time': insitu_time_nearest_str,
    #         'Time_Diff': time_diff_computed_nearest,
    #         'satellite': [self.info['satellite'].upper()]*self.n_mu_total,
    #         'platform': [self.info['platform'].upper()]*self.n_mu_total,
    #         'sensor': [self.info['sensor'].upper()]*self.n_mu_total,
    #         'site': [self.info[key_site].upper()]*self.n_mu_total,
    #         'ac': [self.info['satellite_aco_processor'].upper()]*self.n_mu_total,
    #         'mu_valid': valid_str,
    #         'mu_insitu_id': min_insitu_id,
    #         'spectrum_complete': spectrum_complete,
    #         'n_good_bands': n_valid_bands,
    #         'Ins_Lat': [-999.0]*self.n_mu_total,
    #         'Ins_Lon': [-999.0]*self.n_mu_total,
    #         'status': status
    #     }
    #     self.df_mu = pd.DataFrame(data=data_mu_df)
    #
    #
    #
    #     data_valid_df = {
    ...
    #         'Total': [self.n_mu_total],
    #         'Valid': [nmu_valid],
    #         '# NO VALID MDB FILE': np.sum(status == -1),
    #         '# NO VALID MATCH-UP INDEX': np.sum(status == -2),
    #         '# OUT OF TIME WINDOW': np.sum(status == -3),
    #         '# INVALID IN SITU DATA': np.sum(status == -4),
    #         '# INCOMPLETE IN SITU SPECTRUM': np.sum(status == -5),
    #         '# INVALID SAT DATA': np.sum(status == -6),
    #         '# INVALID IN SITU VALUES': np.sum(status == -7),
    #     }
    #
    #     df_valid = pd.DataFrame(data=data_valid_df)
    #     print(f'[INFO] Summary data frame creation completed.')
    #     print(
    ...
    #             self.index_mu = index_mu
    #             self.mu_sat_time = self.sat_times[index_mu]
    #             self.mu_insitu_time = dt.fromtimestamp(float(self.variables['insitu_time'][index_mu][0])).astimezone(
    #                 timezone.utc)
    #         elif valid_mu_ins[index_mu]==0:
    #             mu_valid = False
    #             info_mu = {
    #                 'status':-4,
    #                 'valid_bands': [False] * len(self.wlref_sat_indices),
    ...
    #             no_insitu = True
    #             continue
    #         index = np.ma.argmin(np.abs(wl - self.insitu_bands))
    #         if len(self.insitu_bands.shape) == 2:  ##more than one instrument
    #             iins, iwl = np.unravel_index(index, self.insitu_bands.shape)
    #             diff = np.abs(wl - self.insitu_bands[iins, iwl])
    #         else:
    ...
    #     if 'insitu_original_bands' in self.dimensions:
    ...
    #
    #
    # def get_insitu_spectra(self,scale_factor,use_rhow):
    #
    #     insitu_spectra = self.nc.variables['insitu_Rrs'][:]
    #     insitu_spectra = np.moveaxis(insitu_spectra,2,1)
    #     nmu = insitu_spectra.shape[0]
    #     nid = insitu_spectra.shape[1]
    #     nbands = insitu_spectra.shape[2]
    #     all_spectra = insitu_spectra.reshape((nmu*nid,nbands))
    #     spectra_with_data = np.ma.count(all_spectra,axis=1) > 0
    #
    #
    #     insitu_valid = self.nc.variables['insitu_valid'][:]
    #     if 'mu_insitu_id' in self.nc.variables and 'mu_valid' in self.nc.variables:
    #         mu_insitu_id = self.nc.variables['mu_insitu_id'][:]
    #         mu_valid = self.nc.variables['mu_valid'][:]
    #         for imu in range(nmu):
    #             if mu_valid[imu]==0:
    #                 continue
    #             insitu_valid[imu,int(mu_insitu_id[imu])]=insitu_valid[imu,int(mu_insitu_id[imu])]+1
    #
    #     insitu_valid = insitu_valid.reshape((nmu*nid))
    #
    #     all_spectra = all_spectra[spectra_with_data,:]
    #     if use_rhow:
    #         all_spectra = all_spectra * np.pi
    #     if scale_factor is not None:
    #         all_spectra = all_spectra * scale_factor
    #
    #     insitu_valid = insitu_valid[spectra_with_data]
    #
    #
    #     return all_spectra,insitu_valid,spectra_with_data
    #
    #
    ...
    #         if len(all_flag_list)==1 and np.isscalar(all_flag_values):
    #             all_flag_values = np.array([all_flag_values])
